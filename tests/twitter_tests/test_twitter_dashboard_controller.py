...
class TestUserPayload:
    ...
    def test_user_payload_retrieval_good(self,client):
    ...
    @patch('analytics_api.services.twitter_dashboard_service.getTwitterUserData', new=AsyncMock())
    def test_user_payload_retrieval_bad(self,client):
       #We need access to our session
        with client.session_transaction() as session:
            #Setting up session variables
            session['twitter_auth_key'] = "abc123" 
